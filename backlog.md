...
## 2026-08-28 — deferred from the anonymity/SMS session

- **Turn on result texts (high).** Everything is built and verified but the flag is off and the sender is unscheduled. Blocked on: upgrading Twilio off trial (a trial account only sends to pre-verified numbers), setting `TWILIO_*` secrets, deploying `send-result-sms`, and scheduling it. Steps are in `docs/RESULT_SMS.md`, including a dry run that shows the message count before anything is billed.
- **Add `SUPABASE_SERVICE_ROLE_KEY` to `.env.local` (high).** Absent locally, so the masked-phone rung of the voter chain (`••• 1694`) silently degrades to `Anonymous #N` in dev, and `closeSquabble()` falls back to the RLS-bound client. Also confirm it is set in Vercel — nothing in this session could verify the production environment. I can't add a secret; this one is yours.
- **A migration-state check that runs in CI (high).** Two migrations sat unapplied for months and nothing noticed. A script that diffs `supabase/migrations/` against `pg_policies` + `information_schema.column_privileges` would have caught both, and would have caught ISSUE-039 being marked fixed while the hole was open.
- **Regression test for the phone grant (medium).** ISSUE-039 has been logged as "regression test still missing" since August. It now *can* be written — the DB is live and the probe is three REST calls (`select=id,phone` must 403 for `anon`, PATCH of `phone` must 403, PATCH of `display_name` must 204). Needs a home: there is no integration-test harness yet.
- **`e2e/` still does not exist (medium).** `pnpm test:e2e` remains a command that errors. Playwright is configured and the flows worth covering are now well understood: anonymous vote → name prompt → dashboard bucketing.
- **STOP-keyword handling for SMS (medium).** The opt-out switch covers the in-app path, but carriers expect a replying "STOP" to work. Needs a Twilio inbound webhook that sets `sms_opt_out`. Required before any real volume.
- **Per-theme badge tints (low).** Status badges use fixed palette colours because theme CSS variables reach `body` but did not resolve for `bg-foreground` on a nested badge — worth understanding rather than working around, but the fixed colours are legible everywhere and the bug they replaced was worse.
- **The remaining `copy-and-features.test.ts` describes (low).** The worst offenders now import real helpers, but several blocks still assert on locally-declared logic (OTP handling, login redirect, rematch clamping). Same treatment: extract the helper, import it in both places.
- **Squabble page still makes ~5 sequential round trips (low).** The independent ones are now parallel, but the voter breakdown and the user's own vote still chain. Median page time is ~130ms against remote Supabase and network variance (stdev up to 48ms) swamps the difference, so this is not worth doing until there's a reason.
